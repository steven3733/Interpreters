...
// COMPILATION
//  g++ x10i-interpreter8.cpp -o x10i-interpreter8.x
...
int main(int argc, char *argv[]) {
    ...
    myfile.open("signals-new.txt");
    readFile();

}

void readFile() {
    cur.setPoint(0.0, 0.0, 0.0);
    //char * line = NULL;
    //size_t len = 0;
    char dataLine[50];
    int sizeLine = 50;
        ...
        if (gc.seen & GCODE_G) {
            ...
            switch (gc.G) {
                ...
                case 0:
                    ...
                    //                    cout << "curX = " << cur.x << "| curY = " << cur.y << "| curZ = " << cur.z << "\n";
                    //                    cout << "nextX = " << next.x << "| nextY = " << next.y << "| nextZ = " << next.z << "\n";
                ...
                case 1:
                    ...
                    //                    cout << "curX = " << cur.x << "| c                    bresenham_3D(cur, next);
                    //                            cur.copy(next);
                    //                    urY = " << cur.y << " | curZ = " << cur.z << "\n";
                    //                            cout << "nextX = " << next.x << "| nextY = " << next.y << "|  nextZ = " << next.z << "\n";
                    bresenham_3D(cur, next);
                    cur.copy(next);
                    break;
                ...
                case 2:
                    drawCircle(cur, next, gc.I, gc.J, 2);
                    cur.copy(next);
                    break;
                case 3:
                    drawCircle(cur, next, gc.I, gc.J, 3);
                    cur.copy(next);
                    break;
