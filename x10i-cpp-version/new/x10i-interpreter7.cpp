...
// COMPILATION
//  g++ x10i-interpreter7.cpp -o x10i-interpreter7.x
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
    ...
    while (!feof(ifp)) {
        ...
        for (ind = 0; ind < sizeLine; ind += (1 + len)) {
            ...
            switch (dataLine[ind]) {
                    ...
                    PARSE_FLOAT('R', &dataLine[ind + 1], len, gc.R, gc.seen, GCODE_R);
                    PARSE_FLOAT('Q', &dataLine[ind + 1], len, gc.Q, gc.seen, GCODE_Q);
                    PARSE_FLOAT('S', &dataLine[ind + 1], len, gc.S, gc.seen, GCODE_S);
                default:
                    break;
            }
        }
