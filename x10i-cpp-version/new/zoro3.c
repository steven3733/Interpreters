...
//  /usr/bin/gcc -pedantic -Wall -O2 -DX10_LINUX_BUILD -std=gnu99 -I../include -L../lib  ../lib/libfflyusb.so ../lib/libxlinecapi.so -lpthread ../lib/unlockioc.o ../lib/linuxkb.o ../lib/xlineauthenticate.o ../lib/xlinecexit.o -lm -o zoro3.x zoro3.c 
...
//const double STEP = 0.0045;      // kalu drawing save dlm dwg ,(drawing 2:1)0.01 standard, 0.005 besar, 0.02 kecik ,(kalu drawing 1:1)0.005025(tepat)
...
void pcline(void *xBoard, double cx2, double cy2, double cz2) {
    ...
    int valout_pulse_wry = 0;
    char valout_pulse_wry_bin[33]; // LAST CHAR IS NULL "\0"
    char status_value_bin[33]; // LAST CHAR IS NULL "\0"
    char control_value_bin[33]; // LAST CHAR IS NULL "\0"
    int valout_dir_wry = 0;
    char valout_dir_wry_bin[33]; // LAST CHAR IS NULL "\0"
    ...
    while (u <= 1.0) {
        ...
        printf("%d valout_pulse = %d\tvalout_dir = %d \n", count, valout_pulse, valout_dir);
...
/*
    int k;
        for (k = 1; k <= line - 1; k++) {
            //printf("%lu\t%lu\t\t%d\t\t%f\t%f\t%f\t", k, codeline, laser[k - 1], XP[k] - XP[k - 1], YP[k] - YP[k - 1], ZP[k] - ZP[k - 1]);

            pcline(xBoard, XP[k] - XP[k - 1], YP[k] - YP[k - 1], ZP[k] - ZP[k - 1]);

        }
*/
    // DISPLAY COLUMN HEADERS
    /*
        printf("j\tdeltaXP[j]\tdeltaYP[j]\tdeltaZP[j]\n");
        for (j = 1; j < line; j++) {
            printf("%lu\t\t%f\t%f\t%f\n", j, XP[j] - XP[j - 1], YP[j] - YP[j - 1], ZP[j] - ZP[j - 1]);
        }
     */
    printf("Converting G code completed...\n");
    fclose(fpData);
